...
class RefreshTokenDTOFactory(factory.Factory):
    ...
    refresh_token_id = factory.Faker("random_int", min=1, max=10000)
    user_id = factory.Faker("uuid4")
...
class AddMealDTOFactory(factory.Factory):
    ...
    date = factory.Faker("date")
    meal_type = factory.Iterator([choice for choice in MealType.get_list_of_values()])
    meal_preference=factory.Iterator([choice for choice in MealPreferenceType.get_list_of_values()])
