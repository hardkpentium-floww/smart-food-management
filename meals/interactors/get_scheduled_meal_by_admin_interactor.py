...
class GetScheduledMealByAdminInteractor:
    ...
    def get_scheduled_meal_by_admin(self,date:datetime.date,meal_type:MealTypeEnum)->AdminScheduledMealDTO:
        meal_id = self.storage.get_meal_id_by_date_and_meal_type(date=date, meal_type=meal_type.value)
        scheduled_meal_dto = self.storage.get_scheduled_meal_by_admin(meal_id=meal_id, date=date, meal_type=meal_type.value)
